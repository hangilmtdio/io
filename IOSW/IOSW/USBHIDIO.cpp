...
CUSBHIDIO::CUSBHIDIO()
{
	m_usDeviceDetected = FALSE;
	m_usDetailDataFlagDone = 0;
}
...
DWORD CUSBHIDIO::GetHIDCollectionDevices(USHORT &NumHIDDevices,
	PSP_DEVICE_INTERFACE_DETAIL_DATA aDetailData[HID_MAXHIDDEVICES],
	PHIDD_ATTRIBUTES aAttributes,
	HIDP_CAPS aNumValueCaps[HID_MAXHIDDEVICES])
{
	PHIDP_PREPARSED_DATA	tmpPreparsedData = NULL;
	LONG					Result;
	...
	*/
	hDevInfo = SetupDiGetClassDevs(&m_HidGuid, NULL, NULL, DIGCF_PRESENT | DIGCF_INTERFACEDEVICE);
	...
	*/
	...
	do
	{
		Result = SetupDiEnumDeviceInterfaces(hDevInfo, 0, &m_HidGuid, HIDDeviceIndex, &devInfoData);
	...
	*/
	...
	if (m_usDetailDataFlagDone > 0 && m_usDetailDataFlagDone < HID_MAXHIDDEVICES)
	{
		for (oldHIDDeviceIndex = 0; oldHIDDeviceIndex < m_usDetailDataFlagDone; oldHIDDeviceIndex++)
		{
			free(aDetailData[oldHIDDeviceIndex]);
		}
	}

	m_usDetailDataFlagDone = HIDDeviceIndex;
	...
	for (HIDDeviceIndex = 0; HIDDeviceIndex < NumHIDDevices; HIDDeviceIndex++)
	{
		...
		if (HID_MAXDEVICEPATH > Length)
		{
			...
			*/
			...
			Result = HidD_GetAttributes(tmpDeviceHandle, &aAttributes[HIDDeviceIndex]);
			...
			*/
			...
			if (!HidD_GetPreparsedData(tmpDeviceHandle, &tmpPreparsedData)) return GetLastError();
			...
			*/
			...
			HidP_GetCaps(tmpPreparsedData, &aNumValueCaps[HIDDeviceIndex]);
	...
	HANDLE tmpDeviceHandle)
{
	...
	*/
	...
	HidP_GetCaps(tmpPreparsedData, &Capabilities);
	...
	HANDLE tmpDeviceHandle)
{
	...
	*/
	...
	if (tmpDeviceHandle == NULL || tmpDeviceHandle == INVALID_HANDLE_VALUE)
	{
		...
		tmpDeviceHandle = CreateFile(aDetailData->DevicePath,
			GENERIC_READ | GENERIC_WRITE,
			FILE_SHARE_READ | FILE_SHARE_WRITE,
			(LPSECURITY_ATTRIBUTES)NULL,
			OPEN_EXISTING,
	...
	HANDLE	tmpReadHandle)
{
	...
	*/
	...
	//	{
	tmphEventObject = CreateEvent(NULL, TRUE, TRUE, _T(""));
	...
	*/
	Result = WaitForSingleObject(tmphEventObject, WaitForMsc);
	...
	HANDLE	tmpReadHandle)
{
	...
	*/
	if (tmpReadHandle == NULL || tmpReadHandle == INVALID_HANDLE_VALUE)
	{
		...
		tmpReadHandle = CreateFile(aDetailData->DevicePath,
			GENERIC_READ,
			FILE_SHARE_READ | FILE_SHARE_WRITE,
			(LPSECURITY_ATTRIBUTES)NULL,
			OPEN_EXISTING,
	...
	*/
	...
	if (!HidD_GetPreparsedData(tmpReadHandle, &tmpPreparsedData))
		return GetLastError();
...
DWORD CUSBHIDIO::SetHIDLEDState(PSP_DEVICE_INTERFACE_DETAIL_DATA aDetailData, HIDP_REPORT_TYPE ReportType, USAGE lcUsagePage, PUSAGE pUsageList, PULONG pUsageLength, HANDLE	tmpWriteHandle)
{
	PHIDP_PREPARSED_DATA	tmpPreparsedData;
	HIDP_CAPS				Capabilities;
	...
	*/
	...
	if (tmpWriteHandle != INVALID_HANDLE_VALUE) {
		Result = WriteFile(tmpWriteHandle, tmpOutputReport, Capabilities.OutputReportByteLength, &NumberOfBytesWritten, NULL);
	}
...
DWORD CUSBHIDIO::SendHIDReport(PSP_DEVICE_INTERFACE_DETAIL_DATA aDetailData, PCHAR pReport, HANDLE tmpWriteHandle)
{
	PHIDP_PREPARSED_DATA	tmpPreparsedData;
	HIDP_CAPS				Capabilities;
	DWORD					Result;
	...
	*/
	if (tmpWriteHandle == NULL || tmpWriteHandle == INVALID_HANDLE_VALUE)
	{
		...
		tmpWriteHandle = CreateFile(aDetailData->DevicePath,
			GENERIC_WRITE,
			FILE_SHARE_READ | FILE_SHARE_WRITE,
			(LPSECURITY_ATTRIBUTES)NULL,
			OPEN_EXISTING,
	...
	*/
	...
	if (!HidD_GetPreparsedData(tmpWriteHandle, &tmpPreparsedData)) return GetLastError();
	...
	*/
	...
	if (tmpWriteHandle != INVALID_HANDLE_VALUE) {
		Result = WriteFile(tmpWriteHandle, pReport, Capabilities.OutputReportByteLength, &NumberOfBytesWritten, NULL);
	}
